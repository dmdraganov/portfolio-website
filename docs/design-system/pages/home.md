...
1. **Fragmented** — hero: modules are separated, representing an unresolved task.
2. **Aligned** — positioning settles: modules find a shared axis as the promise becomes clear.
3. **Exploded view** — projects and capabilities: modules separate with visible relationships, exposing how systems are built.
4. **Resolved** — final CTA: modules lock into one stable object and stop moving.

Transitions between states explain the narrative. The object has no continuous idle rotation.
...
- Final CTA: system object resolves; CTA itself uses only hover and press feedback.
...
- No custom cursor.
- No parallax.
- Static or low-complexity system object.
