...
- Treat `docs/prd.md` as the product contract, `src/content/site.ts` and `src/content/projects.ts` as approved public copy, `docs/CONTENT.md` as its editorial map, `docs/EXPERIENCE.md` as behavior/IA, and `docs/DESIGN.md` plus `docs/design-system/` as visual rules. Do not silently rewrite approved copy.
- Stack: Next.js, TypeScript, Tailwind CSS, Motion, shadcn/ui, ESLint, Prettier.
...
- Import media statically beside its content record so Next.js owns intrinsic dimensions; keep meaningful Russian alternatives and verify files under `public/media` at build time.
- Keep analytics fire-and-forget behind `TrackedLink`/the shared analytics facade; it must never block navigation.
- Deploy through Vercel's Git integration. Production and Preview builds must use Vercel system environment variables, pass the repository quality gate, and keep canonical metadata pointed at the production domain.
